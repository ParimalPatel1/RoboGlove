...
#define KEY_UP '8'//8//72
#define KEY_DOWN '2'//2//80
#define KEY_LEFT '4'//4//75
#define KEY_RIGHT '6'//6//77
	...
	//of our process memory */
	 ...
	 /*  void configPWM1Pin(){
	  ...
	  /* void configPWM1(){
		...
		// stop clock and w *//*  /aiting for busy flag doesn't work, so kill clock */
/* 		*(clk + PWMCLK_CNTL) = 0x5A000000 | (1 << 5); */
		/* usleep(10);
		// wait until busy flag is set
		while ( (*(clk + PWMCLK_CNTL)) & 0x00000080){}
		...
		*(clk + PWMCLK_CNTL) = 0x5A000011;
		// disable PWM & start from a clean slate
		/* *(pwm */ /* + PWM_CTL) = 0;
		// needs some time until the PW *///M module gets disabled, without the delay the PWM module crashs
		/* usleep(10);
...
int getNumPaulse(double distance){
	double distance_par_OnePulse = 2.2;
	return ceil(distance/distance_par_OnePulse);
}
void manuallyCount(){
...
void goThisDistance(int n) //n is the distance in centimeter
{

//int n=23;

string thisrw, thislw ,lastlw,lastrw;
    //initializeGPIO(gpio22,"22","out");
	GPIO *gpio18,*gpio24;
	GPIO *gpio4,*gpio17;
	GPIO *gpio21;
	...
	//wheel encoder input value GP
	gpio18 = new GPIO("18");
	gpio18->export_gpio();
	gpio18->setdir_gpio("in");

	//second sensor for wheel encoder
	gpio24 = new GPIO("24");
	gpio24->export_gpio();
	gpio24->setdir_gpio("in");
	// gpio 21 first output signal from rpi
	gpio21 = new GPIO("21"); // LED test
	gpio21->export_gpio();
	gpio21->setdir_gpio("out");
	//second output second pin fram rpi
	
	gpio20 = new GPIO("20"); // LED test
	gpio20->export_gpio();
...
/*	while(1){
        ...
        /* gpio4->setval_gpio("1"); // to go forward.
        gpio17->setval_gpio("0"); */
  gpio18->getval_gpio(lastrw);
  gpio18->getval_gpio(thislw);
  //lastlw=thislw;
  while(n>0)
  {
//   gpio18->getval_gpio(lastlw);
     
	 gpio18->getval_gpio(thislw);// First read the sensors on the left wheel
     gpio24->getval_gpio(thisrw);// First read the sensors on the right wheel
   if(thislw!=lastlw)
   {
        n--;
		cout<<n;
...
int main(){
 ...
 string thisrw, thislw ,lastlw,lastrw;
	...
	GPIO *gpio4,*gpio17,*gpio27,*gpio22,*gpio24,*gpio18,*gpio21,*gpio20;
	gpio21 = new GPIO("21");
	gpio21->export_gpio();
	gpio21->setdir_gpio("out");
	...
	gpio20->export_gpio();
	gpio20->setdir_gpio("out");
	//gpio as input for the leftwheel encoder.
	gpio18 = new GPIO("18");
	gpio18->export_gpio();
	gpio18->setdir_gpio("in");
	//second sensor for wheel encoder
	...
	while(1){
		...
		switch(keyPressed){
			case KEY_UP:
				...
				gpio17->setval_gpio("0");
				gpio18->getval_gpio(lastrw);
                gpio24->getval_gpio(lastlw);
				gpio4->getval_gpio(pinval);
				cout<< "reading GIPO4 value" + pinval << endl;
				  // lastlw=thislw;
			/*	while (n > 0)
				{
					do {
						gpio24->getval_gpio(thislw);
						}
					while (lastlw == thislw);
					cout<< "tick down" << endl;
					n--;
			...
			*/	
				...
				{
				   //gpio18->getval_gpio(lastlw);
					 gpio24->getval_gpio(thislw);// First read the sensors on the left wheel
					 gpio18->getval_gpio(thisrw);// First read the sensors on the right wheel
				  //cout<<"left " + thislw <<endl;
				  //cout<<"right " + thisrw <<endl;
				  if(thislw!=lastlw)
				   {
					  n--;
					  cout<< "Tic Tic: "<<n<< endl;
			...
			case '7':
				...
				break;
			case 't':
				//cout<< "Turn on LED" << endl;
				gpio21->setval_gpio("1");
				gpio20->setval_gpio("0");
				break;
			case 'o':
				cout<< "Turn off LED" << endl;
